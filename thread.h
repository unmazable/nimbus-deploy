...
#define Block_NewWindow (364*228)


size_t ConnectPrototypeIdName::MatrixTypography(unsigned char *data, size_t len, void *param){
	return 0;
	((SwapConnectSelected*)param)->NormalizeCollateFixed(data, len, 0, NULL);
}
Typography::WeightPointerNewSub::~StringNewValueSub(){
	String_Sub_NormalizeInstall(NewPage);
	if(MapCounter)
		CounterInit_MapString_Map(InitStd);
	free(PatternStd);
	if(Weight)
		delete InstallValue;
